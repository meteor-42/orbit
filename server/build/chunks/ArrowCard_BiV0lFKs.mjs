...
import { f as formatDate } from './PageLayout_BiBushs7.mjs';
...
function ArrowCard({
  ...
  return ssr(_tmpl$, ssrHydrationKey(), `/${escape(entry.collection, true)}/${escape(entry.slug, true)}`, pill && ssr(_tmpl$2, ssrHydrationKey(), entry.collection === "howto" ? "post" : "project"), escape(formatDate(entry.data.date)), escape(entry.data.title), escape(entry.data.summary), escape(entry.data.tags.map((tag) => (
